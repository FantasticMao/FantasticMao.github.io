...
## 格式
**术语说明**：*block-like construct*块状结构 指类的实体、成员或构造器。注意，在后续4.8.3.1[数组初始化](#数组初始化：可以写成块状结构)章节中，任何数组的初始化可被认为是一个块状结构。
...
### 骆驼峰式命名
经常有多种理由需要将英语短语转换为驼峰形式，如首字母缩略词“iOS”或不寻常的结构“IPv6”，Google风格指定了以下几种确定性的方案。

以散文形式命名开始：
1. 将短语转换成纯ASCII编码，并删除所有省略号。例如，“Müller's algorithm”可以转换成“Muellers algorithm”。
