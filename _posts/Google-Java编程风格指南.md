...
#### 空语句块：可以简洁
一个空语句块或块状结构可以遵从K&R风格（如[4.1.2章节](#非空语句块：K-amp-R风格)描述）。或者，可以在左花括号起始之后，没有任何字符和换行，立即右花括号结束。除非它是*multi-block statement*多块语句（一个包含多块的语句，如：`if/else`、`try/catch/finally`）的一部分，否则不允许这样做。
