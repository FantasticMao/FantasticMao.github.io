...
4. 逗号（`,`）紧随它之前的内容。
5. 项链lambda箭头的行永不换行，除非lambda主体仅有单个表达式，且能在lambda箭头后立即出现的情况下。例如：
	```java
...
4.6.3[水平对齐](#水平对齐)章节指出，不鼓励使用可变数目的空格对齐上述说的几行。
...
1. 类中连续的成员或初始化中：字段、构造器、方法、嵌套类、静态初始化和实例初始化。
	* **特殊情况**:两个连续字段（无其它代码）之间的空行是可选的。比如可以使用空行去创建字段间的 *logical groupings*逻辑分组
	* **特殊情况**:枚举常量之间的空行在4.8.1[枚举类](#枚举类)章节介绍。
2. 语句之间，作为逻辑上分组代码的需要
3. 类的第一个成员或者初始化之前，或者最后一个成员或初始化之后。（不鼓励也不反对）
4. 被本文档其他章节中的所要求（如3[源文件结构](#源文件结构)章节和3.3[import语句](#import语句)章节）

多个连续的空行是允许的，但这是不必要也不鼓励的。

#### 水平空格
除了语言、其他规则、语法分隔、注释和Javadoc的需要，单个ASCII空格**仅**出现在如下情况：

1. 分隔保留关键字和它之后的开阔号（`(`），如`if`、`for`、`catch`。
2. 分隔保留关键字和它之前的右花括号（`}`），如`else`、`catch`。
3. 任何左花括号之前（`{`），除了：
	* `@SomeAnnotation({a, b})`（不使用空格）。
	* <code>String[][] x = &#123;&#123;&quot;foo&quot;&#125;&#125;;</code>（<code>&#123;&#123;</code>之间没有空格）。
4. 二进制或三元操作符两侧。这也适用于以下“类似操作符”的符号：
	* 类型约束中的&符：`<T extends Foo & Bar>`
	* 多异常捕获中的|符：`catch (FooException | BarException e)`
	* 增强`for`语句版（“foreach”）的`:`
	* lambda表达式中的箭头：`(String str) -> str.length()`
	但不是
	* 写法类似`Object::toString`的方法引用中的（`::`）
	* 写法类似`Object.toString()`的点分隔符（`.`）
5. `,:;`或者闭括号（`)`）之后
