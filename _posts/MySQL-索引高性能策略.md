...
# 索引的优点
索引可以让服务器快速地定位到表的指定位置，但这不是索引的唯一作用，根据创建索引的数据结构不同，索引也有一些其它的附加作用。

最常见的 B-Tree 索引按照顺序存储数据，所以 MySQL 可以用来做 `ORDER BY` 和 `GROUP BY` 操作。并且 B-Tree 索引中存储了实际的值，所以某些查询只需使用索引就能够完成。
