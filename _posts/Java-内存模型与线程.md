...
一、保证此变量对所有线程的可见性
volatile 变量在被赋值之后，将会使当前 CPU 的高速缓存写入内存，并引起其它 CPU 或者内核无效化它们的高速缓存，从而保证了此变量对所有线程的可见性。<br>
