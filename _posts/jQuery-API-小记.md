...
- `hover()` 方法用来给 mouseenter 和 mouseleave 事件注册处理程序。调用 `hover(f, g)` 就和先调用 `mouseenter(f)` 然后调用 `mouseleave(g)` 一样。如果仅传入一个参数，该参数函数会同时用做 enter 和 leave 事件的处理程序。

- `toggle()` 方法将事件处理程序绑定到单击事件。可以指定两个或多个处理程序函数，当单击事件发生时，jQuery 每次会调用一个处理程序函数。例如，如果调用 `toggle(f, g, h)`，第一次单击事件触发时，会调用函数 `f()`，第二次会调用 `g()`，第三次会调用 `h()`，然后第四次会再调用 `f()`。
...
`load()` 方法可以用来注册 load 事件。如果传给方法的第一个参数是函数而不是字符串，则 `load()` 方法是事件处理程序注册而不是 Ajax 方法。
